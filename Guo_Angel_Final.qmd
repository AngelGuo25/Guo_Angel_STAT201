...
results_BMI <- calc_BMI(168, 110)
sprintf("Your BMI is %s with a value of %.1f.", results_BMI$category, results_BMI$BMI)
...
```{r}
q3_matrix <- cbind(vec1=vec1, vec2=vec2, vec3=vec3)
print(q3_matrix)
```
...
q3_sum <- function(lst){
  ...
  for (i in 1:length(lst)){
    sum_line <- append(sum_line, sum(lst[[i]]))
  }
  return(sum_line)
}
...
q3_log <- function(vec){
  ...
  for (i in 1:length(vec)){
    if (vec[i] %% 2 == 0){
      sum_log <- append(sum_log, TRUE)
    } else{
      sum_log <- append(sum_log, FALSE)
    }
  }
  return(sum_log)
}
...
list_sum <- sapply(q3_list, q3_sum)
list_log <- lapply(list_sum, q3_log)

sum_3 <- q3_sum(q3_list)
log_3 <- q3_log(sum_3)
print(log_3)
...
```{r}
heavy_index <- which.max(fruit_info$weight)
heavy_name <- fruit_info$fruit[heavy_index]
print(heavy_name)
```
