...
```{r}
sprintf("The largest win spread is %d, when the Bears scored %d points and the Opponent scored %d points.", max(bears) - min(opponent), max(bears), min(opponent))
```
...
```{r}
new <- matrix(c(37, 9, 17, 17), ncol=2)
...
```{r}
count <- which(game_score[, "bears"] > game_score[, "opponent"])[1]
losses <- count-1
print(losses)
```
...
```{r}
area <- function(l=48, w=48){
  a_inch <- l*w
  a_ft <- a_inch/144
  return(list(a_inch=a_inch, a_ft=a_ft))
}

area_result <- area(102, 60)
sprintf("The area in square inches is %g.", area_result$a_inch)
sprintf("The area in square feet is %g.", area_result$a_ft)
...
```{r}
q6 <- function(value, check){
  return(value %in% check)
}

q6_results <- q6(value, check)

if (q6_results == TRUE){
  sprintf("%g is in the vector.", value)
} else{
  sprintf("%g is not in the vector.", value)
}
```
